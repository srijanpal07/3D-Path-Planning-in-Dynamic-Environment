...
def run_online_astar(env_path: str,outfile: str, dt: float, n_frames: int, sensor_radius: float, inflation_cells: int, lookahead_cells: int) -> None:
    ...
    for k in range(n_frames):
        ...
        # Never block the cell the probe is currently in (otherwise replanning can fail on contact)
        dyn_mask[probe_idx[0], probe_idx[1], probe_idx[2]] = False

        # Optional: never block the goal either (prevents rare "goal occupied" freeze)
        dyn_mask[goal_idx[0], goal_idx[1], goal_idx[2]] = False
        ...
        if need_plan:
            try:
                ...
                print("Online replanning: no path found; stopping.")
                break
        ...
        # if len(current_idx_path) >= 2:
        #     probe_idx = current_idx_path[1]
        # else:
        #     probe_idx = current_idx_path[0]


        # # Advance to next cell on the path (skip the first entry if it's the current cell)
        # if len(current_idx_path) >= 2:
        #     # Usually path[0] == current, so we move to path[1]
        #     next_idx = current_idx_path[min(1, len(current_idx_path) - 1)]
        # else:
        #     next_idx = probe_idx

        # # Move along the path robustly
        # if current_idx_path is None or len(current_idx_path) == 0:
        #     print("No path returned; stopping.")
        #     break
        ...
        #     next_idx = probe_idx
        # else:
        #     if current_idx_path[0] == probe_idx:
        #         # normal case: path starts at current cell
        #         next_idx = current_idx_path[1] if len(current_idx_path) > 1 else probe_idx
        #     else:
        #         # path doesn't start at current cell; take the first step anyway
        #         next_idx = current_idx_path[0]
        ...
        # probe_idx = next_idx
        # After stepping, we consider that we've consumed one step from the path suffix
        # (next replanning will use current probe_idx anyway)
        # path_cursor = 0
...
def main():
    ...
    parser.add_argument("--outfile", type=str, default="viz.html",help="Output HTML file.")
    ...
    parser.add_argument("--sensor-radius", type=float, default=1.5, help="Sensor radius (m) for discovering dynamic obstacles.")
